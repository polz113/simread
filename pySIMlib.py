...
class pySIMlib:
    ...
    def getNums(self, numFile):
        ...
        for i in range(1, recNum + 1):
            numbers[i] = self.getNum(numFile, i, recLen, nameLen)
        return numbers 
    ...
    def setNum(self, numFile, recNum, recLen, nameLen, name='', number=''):
        ...
        if not name:
            ...
            data = "%s%s%sFFFF" % (self.padString(hexlify(self.ASCII_2_GSM3_38(name)), nameLen << 1, "F"), "%02X" % (len(GSMnumber) / 2), self.padString(GSMnumber, 22, 'F'))
